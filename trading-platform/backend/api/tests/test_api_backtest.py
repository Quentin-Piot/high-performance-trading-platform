...
def test_backtest_ok_200():
    ...
    resp = client.post("/backtest?sma_short=2&sma_long=3", files=files)
...
def test_backtest_invalid_params_400():
    ...
    files = {"csv": ("prices.csv", io.BytesIO(csv.encode("utf-8")), "text/csv")}
    resp = client.post("/backtest?sma_short=5&sma_long=3", files=files)
...
def test_backtest_query_validation_422():
    ...
    files = {"csv": ("prices.csv", io.BytesIO(csv.encode("utf-8")), "text/csv")}
    resp = client.post("/backtest?sma_short=0&sma_long=3", files=files)
