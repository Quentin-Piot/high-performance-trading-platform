        ...
        job_indexes = [
            ...
            },
            {
                ...
                "sql": """
                    ...
                    ON jobs (updated_at DESC, status) WHERE status IN ('running', 'completed', 'failed')
                """,
                "description": "Index for monitoring and cleanup queries"
            },
            {
                "name": "idx_jobs_progress_running",
                "sql": """
                    CREATE INDEX CONCURRENTLY IF NOT EXISTS idx_jobs_progress_running 
                    ON jobs (progress, updated_at DESC) WHERE status = 'running'
                """,
                "description": "Partial index for progress monitoring of running jobs"
            }
        ]
