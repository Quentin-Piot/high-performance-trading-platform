...
class UserRepository:
    ...
    async def create_from_cognito(self, cognito_user: CognitoUser) -> User:
        ...
        try:
            ...
            if existing_user:
                logger.debug(f"Found existing user by cognito_sub after IntegrityError: {existing_user.id}")
                return existing_user
            
            # Try to get by email
            existing_user = await self.get_by_email(cognito_user.email)
            if existing_user:
                logger.debug(f"Found existing user by email after IntegrityError: {existing_user.id}")
                return existing_user
            ...
            logger.error(f"Unexpected error during user creation: {type(e).__name__}: {e}")
            await self.session.rollback()
            raise
    ...
    async def get_or_create_from_cognito(self, cognito_user: CognitoUser) -> User:
        ...
        try:
            logger.debug(f"Starting get_or_create_from_cognito for user: {cognito_user.email}")
            ...
            if user:
                if not user.cognito_sub:
                    logger.debug(f"Found existing user by email without cognito_sub, linking to Cognito: {user.id}")
                    # Link existing user to Cognito (user created with email/password)
                    user.cognito_sub = cognito_user.sub
                    user.name = cognito_user.name
                    user.auth_method = "mixed"  # User can now use both email/password and Google
                    await self.session.commit()
                    await self.session.refresh(user)
                    return user
                else:
                    logger.warning(f"User with email {cognito_user.email} already exists with different cognito_sub. Current: {user.cognito_sub}, New: {cognito_user.sub}")
            ...
            logger.error(f"Error in get_or_create_from_cognito for user {cognito_user.email}: {e}", exc_info=True)
            await self.session.rollback()
            raise
