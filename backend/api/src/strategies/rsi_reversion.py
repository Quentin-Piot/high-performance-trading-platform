...
class RSIReversionStrategy(Strategy):
    ...
    def _rsi(self, series: pd.Series, window: int) -> pd.Series:
        ...
        rs = rs.replace([np.inf, -np.inf], np.nan)  # pyright: ignore[reportAttributeAccessIssue]
        rsi = 100 - (100 / (1 + rs))
        rsi = rsi.fillna(50)
        return rsi
    ...
    def run(self, df: pd.DataFrame, params: RSIParams) -> BacktestResult:  # pyright: ignore[reportIncompatibleMethodOverride]
        ...
        position = raw_signal.shift(1).fillna(0).astype(int) * params.position_size  # pyright: ignore[reportArgumentType]
