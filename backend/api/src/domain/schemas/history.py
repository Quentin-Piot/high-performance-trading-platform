...
class BacktestHistoryCreate(BaseModel):
    ...
    sample_fraction: float | None = Field(None, description="Sample fraction for bootstrap")
    gaussian_scale: float | None = Field(None, description="Gaussian scale for noise")
    datasets_used: list[str] | None = Field(None, description="List of dataset IDs used")
...
class BacktestRerunRequest(BaseModel):
    ...
    history_id: int
    override_params: dict[str, Any] | None = Field(None, description="Parameters to override")
...
class BacktestFavoriteCreate(BaseModel):
    ...
    name: str = Field(..., max_length=255, description="Name for the favorite configuration")
    description: str | None = Field(None, description="Description of the configuration")
    history_id: int = Field(..., description="ID of the backtest history to save as favorite")
