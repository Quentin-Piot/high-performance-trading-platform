...
class TestCognitoService:
    ...
    def test_verify_token_success(self, mock_get_header, mock_decode, mock_cognito_user):
    ...
    def test_verify_token_no_public_key(self, mock_get_header):
        ...
        service = CognitoService()
        service._jwks = {
            "keys": [{"kid": "different-key", "kty": "RSA"}]
        }
...
class TestAuthDependencies:
    ...
    def test_get_current_user_success(self, mock_get_service, client, mock_cognito_user, mock_jwt_token):
        ...
        with patch('api.routers.history.get_user_repo') as mock_user_repo, \
             patch('api.routers.history.get_history_repo') as mock_history_repo:

            mock_user_repo_instance = Mock()
            mock_user_repo_instance.get_or_create_from_cognito = AsyncMock(return_value=Mock(id=1))
    ...
    @patch('core.auth_dependencies.get_cognito_service')
    def test_get_current_user_invalid_token(self, mock_get_service, client, mock_jwt_token):
    ...
    @patch('core.auth_dependencies.get_cognito_service')
    def test_require_verified_email_success(self, mock_get_service, client, mock_jwt_token):
        ...
        assert response.status_code != 403 or "Email verification required" not in response.json().get("detail", "")

    @patch('core.auth_dependencies.get_cognito_service')
    def test_require_verified_email_failure(self, mock_get_service, client, mock_jwt_token):
