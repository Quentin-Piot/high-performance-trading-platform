...
export const useBacktestStore = defineStore("backtest", {
	...
	actions: {
		...
		async runMonteCarloAsync(
			...
			try {
				const submission = await submitMonteCarloAsync(
					files,
					req,
					selectedDatasets,
				);
					...
					const conn = connectMonteCarloProgress(this.mcJobId, {
						onUpdate: (msg: MonteCarloWsMessage) => {
							...
							if (typeof msg.progress === "number") {
								this.mcProgress = Math.max(
									0,
									Math.min(100, msg.progress * 100),
								);
							}
						},
						onError: () => {
				...
				let final: Awaited<ReturnType<typeof getMonteCarloAsyncStatus>> | null =
					null;
				for (let i = 0; i < 120; i++) {
				...
				const isSingleMonteCarloResult = (x: any): boolean => {
					...
					return !!(
						...
						md.sharpe &&
						md.drawdown &&
						Array.isArray(env.timestamps) &&
						Array.isArray(env.median)
					);
				};
				const toResponse = (x: any): any => {
					const startedAt = (final as any).started_at
						? new Date((final as any).started_at)
						: null;
					const completedAt = (final as any).completed_at
						? new Date((final as any).completed_at)
						: null;
