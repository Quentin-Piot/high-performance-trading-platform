...
watch([startDate, endDate, selectedDatasets], () => {
    ...
    const intersectionRange = getIntersectionDateRange(
    ...
    if (!intersectionRange) {
        dateValidationError.value =
            "No valid date intersection found for selected datasets";
        return;
    }
...
watch(selectedDatasets, (newDatasets) => {
    ...
    if (intersectionRange) {
        ...
        if (
            ...
            currentStart > availableEnd
        ) {
            const smartStartDateISO = smartStartDate
                .toISOString()
                .split("T")[0];
        ...
        if (
            !currentEnd ||
            currentEnd > availableEnd ||
            currentEnd < availableStart
        ) {
...
<template>
    <div class="space-y-4">
        ...
        <DatasetSelectionSection v-model:selected-datasets="selectedDatasets" />
