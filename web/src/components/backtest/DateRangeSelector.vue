<template>
  <div class="space-y-4">
    ...
    <div class="grid grid-cols-2 gap-4">
      <!-- Date de début -->
      <div class="space-y-2">
        <Label class="text-xs text-muted-foreground">{{ t('simulate.form.labels.start_date') }}</Label>
        <Popover>
          <PopoverTrigger as-child>
            ...
            >
              ...
              {{ startDateValue ? formatDate(startDateValue) : t('simulate.form.placeholders.select_start_date') }}
            </Button>
          </PopoverTrigger>
      ...
      <!-- Date de fin -->
      <div class="space-y-2">
        <Label class="text-xs text-muted-foreground">{{ t('simulate.form.labels.end_date') }}</Label>
        <Popover>
          <PopoverTrigger as-child>
            <Button
              variant="outline"
            ...
            >
              <CalendarIcon class="mr-2 h-4 w-4" />
              {{ endDateValue ? formatDate(endDateValue) : t('simulate.form.placeholders.select_end_date') }}
            </Button>
          </PopoverTrigger>
          <PopoverContent class="w-auto p-0" align="start">
            <Calendar
