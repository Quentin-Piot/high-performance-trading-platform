...
export function getSymbolDateRange(
	symbol: string,
	dateRanges: DateRangesResponse,
): SymbolDateRange | null {
	return (
		dateRanges.symbols.find(
...
export function getGlobalDateRange(
	dateRanges: DateRangesResponse,
): { min_date: string; max_date: string } | null {
	...
	const latestMinDate = dates.reduce(
		(latest, current) => (current.min > latest ? current.min : latest),
		dates[0]?.min || new Date(),
	);
	const earliestMaxDate = dates.reduce(
		(earliest, current) => (current.max < earliest ? current.max : earliest),
		dates[0]?.max || new Date(),
	);
...
export function getIntersectionDateRange(
	...
	const latestMinDate = symbolRanges.reduce((latest, range) => {
		return new Date(range.min_date) > new Date(latest)
			? range.min_date
			: latest;
	}, symbolRanges[0]?.min_date || "");
	const earliestMaxDate = symbolRanges.reduce((earliest, range) => {
		return new Date(range.max_date) < new Date(earliest)
			? range.max_date
			: earliest;
