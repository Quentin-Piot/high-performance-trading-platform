...
export function createSimulator(initialPrice = 100, params: SimulationParams) {
	...
	const triggerTrade = (side: "BUY" | "SELL", qty: number) => {
		...
		const currentPos = state.positions[params.symbol] || {
			qty: 0,
			avgPrice: px,
		};
		const realized =
			side === "SELL" ? round((px - currentPos.avgPrice) * qty, 2) : undefined;
...
function buildOrderBook(px: number): OrderBook {
	...
	for (let i = 1; i <= levels; i++) {
		bids.push({
			price: round(px - i * tick, 2),
			qty: Math.floor(5 + Math.random() * 95),
		});
		asks.push({
			price: round(px + i * tick, 2),
			qty: Math.floor(5 + Math.random() * 95),
