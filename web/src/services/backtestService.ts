...
export type BacktestApiResponse =
	| BacktestResponse
	| MultipleBacktestResponse
	| MonteCarloResponse;
export function isBacktestResponse(
	response: BacktestApiResponse,
): response is BacktestResponse {
	return (
		"timestamps" in response &&
		"equity_curve" in response &&
		!("results" in response)
	);
}
export function isMultipleBacktestResponse(
	response: BacktestApiResponse,
): response is MultipleBacktestResponse {
	return (
		"results" in response &&
		Array.isArray(response.results) &&
		response.results.length > 0 &&
		!!response.results[0] &&
		"timestamps" in response.results[0]
	);
}
export function isMonteCarloResponse(
	response: any,
): response is MonteCarloResponse {
...
export function validateCsvFile(file: File): void {
	...
	if (file.size > 5 * 1024 * 1024)
		throw new BacktestValidationError("error.csv_too_large");
}
export function validateCsvFiles(files: File[]): void {
	if (files.length === 0)
		throw new BacktestValidationError("error.invalid_csv", "No files provided");
	if (files.length > 10)
...
function buildQuery(
	...
	if (req.monte_carlo_runs)
		params.set("monte_carlo_runs", req.monte_carlo_runs.toString());
	if (req.method) params.set("method", req.method);
	if (req.sample_fraction)
		params.set("sample_fraction", req.sample_fraction.toString());
	if (req.gaussian_scale)
		params.set("gaussian_scale", req.gaussian_scale.toString());
...
function normalizeMultipleResponse(
	resp: RawMultipleBacktestResponse,
): MultipleBacktestResponse {
	return {
		...
		aggregated_metrics: {
			...
			average_drawdown: asNegativeDrawdown(
				resp.aggregated_metrics.average_drawdown,
			),
...
export async function runBacktest(
	...
	const data = await postFormData<RawBacktestResponse>(
		`/backtest${query}`,
		formData,
	);
...
export async function submitMonteCarloAsync(
	...
	req: BacktestRequest,
	selectedDatasets?: string[],
): Promise<MonteCarloAsyncSubmissionResponse> {
	const params = new URLSearchParams();
	if (selectedDatasets && selectedDatasets.length > 0) {
		params.set("symbol", selectedDatasets[0]!);
		if (req.dates?.startDate) params.set("start_date", req.dates.startDate);
